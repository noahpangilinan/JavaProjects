...
public class ToyLand {
    ...
    public void playTime() {
        ...
        while(activeToys.size() > 0){
            activeToys.get(0).play(getRandomTime());
            if(activeToys.get(0).isRetired()) {
                retiredToys.add((activeToys.get(0)));
                activeToys.remove(0);
            }
            else{
                Collections.rotate(activeToys, -1);
            }
    ...
    public void displayStatistics() {
        int happiness = 0;
        double wear = 0.0;
        double size = retiredToys.size();
        System.out.println("Retired toys:");
        for(int i=0; i < retiredToys.size(); i++){
            System.out.println("\t" + retiredToys.get(i).toString());
            happiness += retiredToys.get(i).getHappiness();
            wear += retiredToys.get(i).getWear();
        }
        System.out.println("Total toys: " + retiredToys.size());
        System.out.println("Total happiness: " + happiness);
        System.out.println("Average wear: " + (wear/size));
    }
